...
- Stable release: [v0.2.0](https://github.com/patrick-fu/cpa-codex-compact-bridge/releases/tag/v0.2.0), published for linux/amd64, macOS/arm64, and macOS/amd64 with one sha256sum-compatible checksum file.
...
Reload or restart CPA, then confirm `GET /v0/management/plugins` reports version `0.2.0`, `registered: true`, and `effective_enabled: true`. File discovery alone may select the new path before the already loaded plugin instance is replaced.
...
Install the extracted dylib under `<plugin-dir>/darwin/<arm64|amd64>/cpa-codex-compact-bridge-v0.2.0.dylib`.
