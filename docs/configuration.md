...
The bridge accepts a summary only when its terminal response shape has usable assistant `output_text` and no tool-call output. A failed or incomplete terminal state, a chat completion that does not end with `finish_reason: stop`, an incomplete-details marker, tool calls, empty text, or an over-limit summary fails with retryable `compact_bridge_failed`; no partial summary is written into the session. CPA versions through `7.2.125` synthesize `status: "completed"` for bridged non-streaming Responses replies, so that status is not a truncation signal there; from `7.2.147`, a truncated response is represented as `status: "incomplete"` with an incomplete reason and is rejected. In every version, tool calls are rejected from their output shape rather than status.
