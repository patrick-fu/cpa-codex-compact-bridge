...
V1 and V2 share the same Bridge summarization function and both persisted a valid `cpa_compact_*` item. The repeats therefore do not indicate a V1/V2 response-shape regression. They expose a quality problem in the current one-shot generic handoff prompt: there is no required fact checklist, output contract, or validation/retry gate. Summary output was also variable: V1 summaries were 1,400–1,487 characters; V2 summaries were 2,397–3,375 characters; the local summary was 1,782 characters. More text did not guarantee the correct decision.

Minimum next fix: strengthen only the Bridge summary contract and its tests. Require explicit preservation of identifiers, exact values, completed-versus-pending status, chronological failure/retry distinctions, and current acceptance criteria; add a regression fixture based on this rollout and fail the semantic gate if either judge drops below 90. Do not change `cmp_*` recognition or fail-closed behavior as part of that fix.
