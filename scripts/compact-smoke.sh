...
item_done=$(grep -c 'response\.output_item\.done' "$body" || true); ids=$(grep -Eo '"id"[[:space:]]*:[[:space:]]*"cpa_compact_[^"]*"' "$body" | sed 's/.*"cpa_compact_/cpa_compact_/; s/"$//' | sort -u); id_count=$(printf '%s\n' "$ids" | grep -c . || true); compact_id=$(printf '%s\n' "$ids" | head -n 1); failed=0; grep -q 'response.failed' "$body" && failed=1 || true; completed=0; grep -q 'response.completed' "$body" && completed=1 || true
