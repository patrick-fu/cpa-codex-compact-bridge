...
compact_id=$(grep -Eo '"id"[[:space:]]*:[[:space:]]*"cpa_compact_[^"]*"' "$body" | head -n 1 | sed 's/.*"\(cpa_compact_[^"]*\)"/\1/'); encrypted=$(grep -Eo '"encrypted_content"[[:space:]]*:[[:space:]]*"([^"\\]|\\.)*"' "$body" | head -n 1 | sed 's/^[^:]*:[[:space:]]*"//; s/"$//'); enc_len=$(printf '%s' "$encrypted" | wc -c | tr -d ' '); ascii=0; [ -n "$encrypted" ] && LC_ALL=C printf '%s' "$encrypted" | grep -Eq '^[ -~]+$' && ascii=1 || true
bad=""; [ "$status" = 200 ] || bad="http_status"; [ "$count" = 1 ] || bad="${bad:+$bad,}compaction_item_count"; case "$compact_id" in cpa_compact_*) :;; *) bad="${bad:+$bad,}compaction_id";; esac; [ "$completed" -eq 1 ] || bad="${bad:+$bad,}response_completed"; [ "$failed" -eq 0 ] || bad="${bad:+$bad,}response_failed"; [ "$ascii" -eq 1 ] && [ "$enc_len" -ge 1 ] && [ "$enc_len" -le 1048576 ] || bad="${bad:+$bad,}encrypted_content"
