...
usage() { echo "Usage: $0 --base-url URL --api-key KEY --model NAME [--timeout SEC] [--curl-bin PATH] [--expect-failure]"; }
while [ "$#" -gt 0 ]; do
 case "$1" in
...
if [ "$expect_failure" -eq 1 ]; then
 cat >"$payload" <<EOF
{"model":"$model","stream":false,"input":[{"type":"message","role":"user","content":[{"type":"input_text","text":"$sentinel"}]},{"type":"compaction_trigger"},{"type":"message","role":"user","content":[{"type":"input_text","text":"trigger is intentionally not final"}]}]}
EOF
...
count=$(grep -o '"type"[[:space:]]*:[[:space:]]*"compaction"' "$body" | wc -l | tr -d ' '); failed=0; grep -q 'response.failed' "$body" && failed=1 || true; completed=0; grep -q 'response.completed' "$body" && completed=1 || true
compact_id=$(sed -n 's/.*"type"[[:space:]]*:[[:space:]]*"compaction"[^}]*"id"[[:space:]]*:[[:space:]]*"\([^"]*\)".*/\1/p' "$body" | head -n 1); encrypted=$(sed -n 's/.*"encrypted_content"[[:space:]]*:[[:space:]]*"\([^"]*\)".*/\1/p' "$body" | head -n 1); enc_len=$(printf '%s' "$encrypted" | wc -c | tr -d ' '); ascii=0; [ -n "$encrypted" ] && LC_ALL=C printf '%s' "$encrypted" | grep -Eq '^[ -~]+$' && ascii=1 || true
bad=""; [ "$status" = 200 ] || bad="http_status"; [ "$count" = 1 ] || bad="${bad:+$bad,}compaction_item_count"; case "$compact_id" in cpa_compact_*) :;; *) bad="${bad:+$bad,}compaction_id";; esac; [ "$completed" -eq 1 ] || bad="${bad:+$bad,}response_completed"; [ "$failed" -eq 0 ] || bad="${bad:+$bad,}response_failed"; [ "$ascii" -eq 1 ] && [ "$enc_len" -ge 1 ] && [ "$enc_len" -le 1048576 ] || bad="${bad:+$bad,}encrypted_content"; printf '%s' "$encrypted" | grep -q "$sentinel" || bad="${bad:+$bad,}sentinel"
if [ -z "$bad" ]; then echo "PASS compact smoke: HTTP $status; compaction item=$compact_id; encrypted_content_bytes=$enc_len"; exit 0; fi
